...
Subsequently, a Floyd-Warshall shortest path metric is computed to gauge distances between nodes.
...
inf = float('inf')
distance_matrix = [[inf if i != j else 0 for j in range(num_nodes)] for i in range(num_nodes)]
predecessor_matrix = [[None] * num_nodes for _ in range(num_nodes)]
distance_matrix_copy = [[inf if i != j else 0 for j in range(num_nodes)] for i in range(num_nodes)]

# Find the largest edge in the graph and round it up
largest_edge = max(G.edges(data=True), key=lambda x: x[2]['weight'])
largest_edge = math.ceil(largest_edge[2]['weight'])

# Set the distance matrix based on the existing edges in the graph
# 'u' and 'v' represent the nodes connected by the current edge in the graph
for u, v, data in G.edges(data=True):
    ...
    distance_matrix[u_index][v_index] = largest_edge - weight
    distance_matrix_copy[u_index][v_index] = largest_edge - weight
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            if distance_matrix[i][k] + distance_matrix[k][j] < distance_matrix[i][j]:
                distance_matrix_copy[i][j] = distance_matrix[i][k] + distance_matrix[k][j]
                predecessor_matrix[i][j] = predecessor_matrix[k][j]
            else:
                distance_matrix_copy[i][j] = distance_matrix[i][j]

    # Update distance_matrix with the values from distance_matrix_copy
    distance_matrix = np.copy(distance_matrix_copy)
...
# to get low weightS in the case of high connection and high weights for low connection to use in the Floyd Warshall algorithm
G2 = nx.Graph()
        ...
        for other_predictions in all_other_predictions:
            for other_prediction in other_predictions[0]:
                if folder != other_folder and prediction in other_prediction:
                    try:
                        if folder_most_common_predictions[folder] == other_prediction[1]:
                            weight += 1 - other_prediction[2]
                    except Exception as e:
                        print(f"Error processing prediction: {e}")
                        
        # Check if the edge already exists
        if weight != 0:
            if G2.has_edge(folder_most_common_predictions[folder], folder_most_common_predictions[other_folder]):
                G2[folder_most_common_predictions[folder]][folder_most_common_predictions[other_folder]]["weight"] += weight
            else:
                G2.add_edge(folder_most_common_predictions[folder], folder_most_common_predictions[other_folder], weight=weight)

from scipy.cluster.hierarchy import dendrogram
from sklearn.cluster import AgglomerativeClustering

distance_matrix = nx.floyd_warshall(G2)
...
edge_weights = np.array([largest_edge - d['weight'] for u, v, d in G.edges(data=True)])

# Perform Agglomerative Clustering
model = AgglomerativeClustering(metric=shortest_distance_matrix, distance_threshold=0, n_clusters=None)
...
plt.xlabel('Nodes')
plt.ylabel('Distance')
plt.xticks(range(num_nodes), list(G.nodes()))  # Set x-axis ticks to node names
plt.show()
